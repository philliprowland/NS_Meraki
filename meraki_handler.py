...
def processOrg(org):
    ...
    with requests.Session() as s:
        ...
        # Login Session and Redirect to this Org
        print(G + "Processing " + org['name'] + W)
        org['host'] = orgLogin(s, org)

        # Enable API access
        #orgEnableAPI(s, org)

    print(org['name'] + ": Enable API")
    print(org['name'] + ": Get License Detail")
    print(org['name'] + ": Get Change Logs")
    print(org['name'] + ": Get Security Events")
    print(org['name'] + ": API - Admins")
    print(org['name'] + ": API - Get Network Lists")
    
    print(org['name'] + ": Net" + str(1) + ": Get Security Filtering")
    print(org['name'] + ": Net" + str(1) + ": API - Network Alerts")
    print(org['name'] + ": Close Session")
    print(org['name'] + ": Append Results")
    print(org['name'] + ": Done!")

def orgLogin(s, org):
    # Follow the Org Redirect
    ...
    try:
        org_redirect = s.get(host + '/login/org_choose?eid=' + org['id'])
...
def orgEnableAPI(s, org):
    print(org['name'] + ": Verifying API Access")
    try:
        url = 'https://' + org['host'] + '/o/' + org['id'] + '/manage/organization/edit'
        ...
        if str(api_status) == 'None':
            print(Y + org['name'] + ": Enabling API Access" + W)
            url = 'https://' + org['host'] + '/o/' + org['id'] + '/manage/organization'
            token = soup.find('', {"name": "authenticity_token"}).get('value')
            logging.debug("Authenticity Token: " + token)
            payload = {
...
def processMSP():
    ...
    # Use 'with' to ensure the session context is closed after use.
    with requests.Session() as s:
        b_repeat = True # Setup initial loop starting condition
        while b_repeat:
            ...
            for item in soup.find_all('li'):
                org = {}
                org_a = item.find('a')
                org['name'] = org_a.get_text()
                org['id'] = org_a.get('href').split('=', 1)[1]
                org_ids.append(org)
            ...
            b_repeat = False  # Setup escape from while as default action
            try:
                org_redirect = s.get(host + '/login/org_choose?eid=' + (org_ids[0])['id'])
            ...
            if b_repeat: 
                # Wait 2 minutes for Meraki to catch up with any orgs we've accepted and then check for new orgs again
                print("Waiting 2 minutes for Meraki to catch up with new Orgs before continuing...")
                time.sleep(120)  

    return org_ids
def main():
    global password, apikey
    ...
    if apikey is None:
        logging.warning("{0}The keyring 'merakiapi' password is not set")
        sys.exit(2)

    # Accept Invitations
    listOrgs = processMSP()
    logging.debug("{0}Return Value: {1}{2}".format(P, W, str(listOrgs)))

    # Run all Organizations as threads
    pool = ThreadPool(10)
    results = pool.map(processOrg, listOrgs)
    pool.close()
    pool.join()
...
if __name__ == "__main__":
    ...
    global json_admins, str_date, changelog_file, i_timelapse, actions, username
